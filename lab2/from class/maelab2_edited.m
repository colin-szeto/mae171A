...
while typo==1,
    if exist('encoderout')==1,
        encoderoutep=1;%input(['- Specify encoder output (1,2) [' num2str(encoderout) ']: ']);
        if ~isempty(encoderoutep),
            encoderout=encoderoutep;
        end
    else
        encoderout=1;%input(['- Specify encoder output (1,2): ']);
...
while cantfindit,
    ...
    while typo==1,
        ...
        if exist('parfile')==1,
            ...
            parfile='parameters';%input('- Name of file  (between ''s) with model parameters [''parameters'']: ');
            if isempty(parfile),
                parfile='parameters';
            end       
        end
...
while typo==1,
    if exist('DOFs')==1,
        DOFsep=2;%input(['- Specify degrees of freedom (1,2) [' num2str(DOFs) ']: ']);
        if ~isempty(DOFsep),
            DOFs=DOFsep;
        end
    else
        DOFs=2;%input(['- Specify degrees of freedom (1,2): ']);
        while isempty(DOFs),
            clear DOFs
            DOFs=2;%input(['- Specify degrees of freedom (1,2): ']);
        end
...
while typo==1,
    if exist('signofsystem')==1,
        signofsystemep=1;%input(['- Specify sign of encoder (1,-1) [' num2str(signofsystem) ']: ']);
        if ~isempty(signofsystemep),
            signofsystem=signofsystemep;
        end
    else
        signofsystem=1;%input(['- Specify sign of encoder (1,-1): ']);
        while isempty(signofsystem),
            clear signofsystem
            signofsystem1;%=input(['- Specify sign of encoder (1,-1): ']);
        end
    end
...
%
%[C_pi_2,info] = pidtune(G,'PID2',15) % PID tune

[C_pi_2,info] = pidtune(G,'PID2',15.5) % PID tune

[C_pi_2,info] = pidtune(G,'PID2',15.25) % PID tune
% figure;
% 
% syms s
% 
...
if option==2,
   ...
   if exist('stepsize')==1,
      ...
      stepsize=0.6; %input('Enter (open loop) step size [Volts]: ');
   ...
   if exist('dwelltime')==1,
      ...
      dwelltime=5000; %input('Enter dwell time [msec]: ');
   ...
   while typo==1,
      ...
      if exist('sfilename')~=1,
        sfilename='test_2_07-Feb-2025 11_21_31_trim.mat';%input('Name of file (between ''s) that contains open-loop step experiment [ENTER for none]: ');
      else
        osfilename=sfilename; 
        sfilename='test_2_07-Feb-2025 11_21_31_trim.mat';%input(['Name of file (between ''s) that contains open-loop step experiment [ENTER for ''' sfilename ''' or 0 for none]: ']);
